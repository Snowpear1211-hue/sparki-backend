...
function loadData() {
  try {
    ...
    return {
      users: [{ id: 'user_001', name: 'Sparki', gold: 100 }],
      tasks: [],
      tasklists: [],
      expenses: [],
      shopItems: []
...
app.get('/', (req, res) => {
  res.json({ ok: true, version: '5.0', tasks: db.tasks.length });
});

app.get('/api/user', (req, res) => {
  const u = db.users[0];
  res.json({ id: u.id, name: u.name, gold: u.gold, streak_days: u.streak_days, max_streak: u.max_streak });
});
...
app.get('/api/feishu/oauth/callback', async (req, res) => {
  ...
  try {
    const r = await fetch(FEISHU_API + '/authen/v1/oidc/access_token', {
      ...
      body: JSON.stringify({ grant_type: 'authorization_code', code: req.query.code, app_id: APP_ID, app_secret: APP_SECRET })
    });
    const d = await r.json();
    if (d.code !== 0 || !d.data || !d.data.access_token) {
      return res.redirect('https://aqjsoa7d7jhfm.ok.kimi.link/#/?oauth=error&message=' + encodeURIComponent(d.msg || 'fail'));
    }
...
app.post('/api/feishu/sync', async (req, res) => {
  try {
    if (!userTokenCache.token || Date.now() >= userTokenCache.expireAt) {
      return res.status(401).json({ error: 'Need auth first' });
    }

    const lr = await fetch(FEISHU_API + '/task/v2/tasklists?page_size=500', {
      headers: { Authorization: 'Bearer ' + userTokenCache.token }
    });
    const ld = await lr.json();
    if (ld.code !== 0) return res.status(500).json({ error: ld.msg || 'tasklists fail' });

    const lists = (ld.data && ld.data.items) ? ld.data.items : [];
    db.tasklists = lists.map(l => ({ id: l.guid, name: l.name || '未命名', feishu_guid: l.guid, user_id: 'user_001', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }));
    ...
    for (const list of lists) {
      ...
      for (const item of td.data.items) {
        ...
        if (!guid) continue;
        if (db.tasks.find(t => t.feishu_guid === guid)) continue;
        db.tasks.push({
          id: 'feishu_' + Date.now() + '_' + count,
          title: task.summary || '未命名',
          description: task.description || '',
          status: task.completed ? 'done' : 'todo',
          source: 'feishu',
          feishu_guid: guid,
          feishu_tasklist_guid: list.guid,
          user_id: 'user_001',
          gold_reward: 5,
          difficulty: 'medium',
          due_date: task.due && task.due.timestamp ? new Date(parseInt(task.due.timestamp)).toISOString().split('T')[0] : null,
